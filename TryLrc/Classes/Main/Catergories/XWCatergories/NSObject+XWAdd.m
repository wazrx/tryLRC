...
+ (NSArray *)xw_getAllClassMethodsNames {
    return [objc_getMetaClass([NSStringFromClass(self) UTF8String]) xw_getAllInstanceMethodsNames];
}

- (void)xw_setAllNSStringPropertyWithString:(NSString *)string {
    NSArray *attributes = [[self class] _xw_getAllPropertyAttributesInClass];
    [attributes enumerateObjectsUsingBlock:^(NSString *attributeString, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([NSObject _xw_propertyWithAttribute:attributeString isClass:[NSString class]]) {
            NSString * propertyName = [NSObject _xw_getPropertyNameWithAttribute:attributeString];
            SEL setterMethod = [NSObject _xw_getSetterMethodWithProertyName:propertyName];
...
+ (NSArray *)_xw_getAllPropertyAttributesInClass{
    NSMutableArray *allNames = @[].mutableCopy;
    unsigned int propertyCount = 0;
    objc_property_t *propertys = class_copyPropertyList(self, &propertyCount);
    for (int i = 0; i < propertyCount; i ++) {
...
+ (BOOL)_xw_propertyWithAttribute:(NSString *)attributeString isClass:(Class)className{
    NSString *classSring = NSStringFromClass(className);
    if ([[NSObject _xw_getPropertyClassNameWithAttribute:attributeString] isEqualToString:classSring]) {
        return YES;
    }
    return NO;
}

+ (NSString *)_xw_getPropertyClassNameWithAttribute:(NSString *)attributeString{
...
+ (NSString *)_xw_getPropertyNameWithAttribute:(NSString *)attributeString{
    if (!attributeString.length) {
        return nil;
    }
...
+ (SEL)_xw_getSetterMethodWithProertyName:(NSString *)proertyName{
